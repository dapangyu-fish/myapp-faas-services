"""广场社区 FaaS 后端：公开论坛（板块 / 主题 / 楼中楼回帖 / 广场动态）+ 用户主页 + 好友 + 私信。
...
INDENT_PX = 16          # 每层楼中楼缩进像素（客户端直接当 paddingLeft 用）
INDENT_CAP = 12         # 数据无限层级，视觉缩进封顶防溢出
COLLAPSE_AFTER = 2      # 一组楼中楼超过 2 条就默认收起，给「展开」按钮
...
def list_boards():
    ...
    base = (
        "SELECT b.id, b.name, b.intro, b.icon, b.created_at, COALESCE(p.display_name,'') AS owner_name, "
...
def create_board():
    ...
    row = myapp_db.queryone(
        "INSERT INTO boards (name, intro, icon, owner_id) VALUES (%s, %s, %s, %s) "
        "RETURNING id, name, intro, icon, created_at",
        [name, str(body.get("intro") or "").strip()[:200], str(body.get("icon") or "forum").strip()[:48], me],
    )
...
def get_board():
    ...
    b = myapp_db.queryone(
        "SELECT b.id, b.name, b.intro, b.icon, b.owner_id, b.created_at, COALESCE(p.display_name,'') AS owner_name "
...
def feed():
    rows = myapp_db.query(
        "SELECT t.id, t.board_id, t.author_id, t.title, t.body, t.reply_count, t.created_at, "
        "COALESCE(b.name,'') AS board_name, COALESCE(b.icon,'forum') AS board_icon, "
        "COALESCE(p.display_name,'') AS author_name "
        "FROM threads t "
        "LEFT JOIN boards b ON b.id = t.board_id "
        "LEFT JOIN profiles p ON p.owner_id = t.author_id "
...
def _flatten_thread(rows, expanded):
    ...
    - 楼层（parent_id 为 NULL，is_root）始终全显示。
    - 任何一组楼中楼超过 COLLAPSE_AFTER 条且其父 id 不在 expanded 里 → 只显示前 2 条，
      追加一个 kind='more' 行（带剩余条数），客户端渲染「展开 N 条」按钮。
    - 父 id 在 expanded 里 → 全显示，追加一个 kind='collapse' 行（「收起」按钮）。
    迭代式 DFS（不递归），避免超深楼中楼链把调用栈打爆。
    """
    expanded = set(expanded or [])
    children = {}
    for r in rows:
        children.setdefault(r.get("parent_id"), []).append(r)  # 已按 created_at ASC
    ...
    while stack:
        ...
        if kind == "node":
            ...
            node["floor_label"] = "楼主" if depth == 0 else "楼中楼"
            ordered.append(node)
...
def get_thread():
    ...
    return jsonify({"thread": t, "posts": _flatten_thread(rows, expanded)})
...
@app.get("/user")
def get_user():
    """某用户（组内假名）的公开主页：显示名 + 统计 + 最近主题 + 与我的关系。"""
    ...
    threads = myapp_db.query(
        "SELECT t.id, t.title, t.reply_count, t.created_at, COALESCE(b.name,'') AS board_name "
    ...
    return jsonify({
        "user": {"owner_id": uid, "display_name": _name_of(uid),
...
def list_friends():
    ...
    for r in rows:
        oid = r.get("owner_id")
        friends.append({"owner_id": oid, "display_name": _name_of(oid)})
...
def list_requests():
    ...
    reqs = [{"owner_id": r.get("requester_id"), "display_name": _name_of(r.get("requester_id")),
...
def dm_history():
    ...
    return jsonify({"peer": {"owner_id": peer, "display_name": _name_of(peer)}, "messages": rows})
